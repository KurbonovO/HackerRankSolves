...
representing the starting position of each interval (0-indexed). If there are multiple answers, return the lexicographically smallest one.
Input: [1,2,1,2,6,7,5,1], 2		Output: [0, 3, 5]
...
public class MaximumSumOfThreeNonOverlappingSubarrays {
	...
	public int[] maxSumOfThreeSubarrays(int[] nums, int k) {
		...
		for (int i = k; i <= n - 2 * k; i++) {
			...
			int tot = (sum[i + k] - sum[i]) + (sum[l + k] - sum[l])
					+ (sum[r + k] - sum[r]);
			if (tot > maxsum) {
				maxsum = tot;
				ans[0] = l;
