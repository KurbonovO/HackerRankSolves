...
public class SwapAdjacentInLRStringD {
	...
	public boolean canTransform(String start, String end) {
		...
		while (p1 < start.length() && p2 < end.length()) {
			...
			// if the character is 'L', it can only be moved to the left. p1 should be
			// greater or equal to p2.
			if (start.charAt(p1) == 'L' && p2 > p1) {
				return false;
			}
			// if the character is 'R', it can only be moved to the right. p2 should be
