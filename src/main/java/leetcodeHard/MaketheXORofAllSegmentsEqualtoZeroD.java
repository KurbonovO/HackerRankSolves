...
public class MaketheXORofAllSegmentsEqualtoZeroD {

	public int minChanges(int[] nums, int k) {
		int n = nums.length;
		int[][] freq = new int[k][1024];
		// freq[i][j] : freq of j occuring in indeces : m*k+i, we only bother abt 1st k
		// elements bcz remaining array is determined by this
		for (int i = 0; i < n; i++) {
			freq[i % k][nums[i]]++;
		}
		...
		for (int i = 0; i < k; i++) {
			...
			for (int j = 0; j < 1024; j++) {
				...
				/*
				 * we used bestUptoLast bcz we can achieve any value from any value by xoring it
				 * with any-one number, so it becoms : bestUptoLast+cntOfPos*1 (since the new
				 * value is not present in any index, it costs cntOfPos to change all its
				 * indeces)
				 */
