...
/*26 Given a sorted array nums, remove the duplicates in-place such that each
...
public class RemoveDuplicatesFromSortedArray {
	public int removeDuplicates(int[] arrayNumbers) {
		
		int intLenght = 0;
		
		for (int i = 1; i < arrayNumbers.length; i++) {
			
			if (arrayNumbers[i] != arrayNumbers[intLenght])
				arrayNumbers[++intLenght] = arrayNumbers[i];
		}
		return intLenght + 1;
	}
}
