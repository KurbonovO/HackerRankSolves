...
/*743 There are N network nodes, labelled 1 to N. Given times, a list of travel
times as directed edges times[i] = (u, v, w), where u is the source node, v is
the target node, and w is the time it takes for a signal to travel from source
to target. Now, we send a signal from a certain node K. How long will it take
...
public class NetworkDelayTime {
	public int networkDelayTime(int[][] times, int N, int K) {
		...
		// store the source node as key. The value is another map of the neighbor nodes
		// and distance.
